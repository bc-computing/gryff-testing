...
var forceLeader = flag.Int("l", -1, "Force client to talk to a certain replica. Does not work in this version.")
...
func simulatedClientWriter(orInfo *outstandingRequestInfo, readings chan *response, leader int, clientId int) {
	client := createClient(int32(clientId), int32(clientId % 5))
	//var opString string
	var opType OpType
	var k int64
